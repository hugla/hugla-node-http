...
class HuglaHttp {
	...
	setup(callback) {
		...
		// middleware actions
		for (let action of this.middlewareActions) {
			action(app, http);
		}
		...
		for (let uri of Object.keys(assetRoutes)) {
		...
		// controllers
		for (let description of config.controllers || []) {
	...
	close(callback) {
		...
		for (let action of this.shutdownActions) {
			action();
		}
	...
	registerController(parentRouter, description) {
		...
		for (let description of description.children || []) {
			this.registerController(router, description);
		}
